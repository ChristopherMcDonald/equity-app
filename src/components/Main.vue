    ...
    <v-row no-gutters>
      ...
      <v-col class="calc">
        ...
        <div v-for="(player, i) in players" v-bind:key="i">
          <h3>{{player.name}}</h3>
          <v-progress-linear
            v-model="player.winRate"
            :color="getColor(player.winRate)"
            height="25"
          >
...
export default class Main extends Vue {
  players: { name: string; range: string[]; winRate: number }[] = [{
    name: 'Chris',
    range: ['AA', 'KK'],
    winRate: 0.0,
  }, {
  ...
  addPlayer() {
    this.players.push({ name: `Player ${this.players.length + 1}`, range: [], winRate: 0.0 });
  }
  ...
  getEquity() {
    ...
    for (let games = 0; games < 100; games += 1) {
      ...
      holdings.forEach((holding) => {
        const acceptableHands = holding.range.filter((h) => (
          ![h.substring(0, 2), h.substring(2, 4)].some((j) => takenCards.includes(j))
        ));
      ...
      OddsCalculator.calculate(
        ...
        holdings[i].equities.push(equity.getEquity() + equity.getTiePercentage());
      });
    }

    holdings.forEach((holding, i) => {
      this.players[i].winRate = holding.equities.reduce((a, b) => a + b, 0)
      / holding.equities.length;
    });
  }
}
</script>
